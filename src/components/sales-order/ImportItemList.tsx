...
const ImportItemList = () => {
    ...
    const total = salesOrder.detail
        .map((row) => new Decimal(row.QuantityOrdered).times(row.UnitPrice).toString())
        .reduce((pv, currentValue) => new Decimal(currentValue).add(pv).toString(), new Decimal(0).toString());
